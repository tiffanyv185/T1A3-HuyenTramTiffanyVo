...
def view_monthly_report(
    data,
):  # allow users to view total contributions from a chosen month
    month = input(
        "Enter the month for the report (YYYY-MM): "
    )  # prompt user to enter the month for the report
    table = PrettyTable(
        ["Goal", "Total Contributions"]
    )  # initialise 'PrettyTable' with headers - Goal and Total Contributions
    report = (
        {}
