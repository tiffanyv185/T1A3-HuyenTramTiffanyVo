...
def edit_goal(
    ...
    if 0 <= goal_index < len(
        ...
        goal["name"] = (
            input(f"Enter the new name (current: {goal['name']}): ") or goal["name"]
        )  # prompt user to enter in new name or keep current name
        goal["target"] = float(
            input(f"Enter the new target (current: {goal['target']}): ")
            or goal["target"]
        )  # prompt user to enter in new target or keep current target
        goal["deadline"] = (
            input(f"Enter the new deadline (current: {goal['deadline']}): ")
            or goal["deadline"]
        )  # prompt user to enter in new deadline or keep current deadline
        data_manager.save_data(data)  # save the updated data
...
# List goals, prompt user to select which goal to delete.
def delete_goal(data):
    list_goals(data)  # list all current goals
    goal_index = int(
        input("Enter the goal index to delete: ")
    )  # prompt user to enter in index that they want to delete
    if 0 <= goal_index < len(
            data["goals"]):  # ensure that entered index is valid
...
def list_goals(data):  # display all goals in formatted table for user
    # initialise 'PrettyTable' with the headers - Name, Target, Deadline,
    # Saved, Progress
    table = PrettyTable(
        ["Index", "Name", "Target", "Deadline", "Saved", "Progress"])
    for index, goal in enumerate(
        data["goals"]
    ):  # loop through each goal and add its details to the table
